...
echo $count
   ...
   if [ $DIRSIZE -lt 100000000 -o ! $count -lt $fileCount ]
      then
       ...
       # IF NECESSARY USE THE COMMAND LINE FOR ZIPPING: zip -r ../../zips/filename.zip *.tif
       else
           	echo "count02 $count"
           	echo "filecount02 $fileCount"
           # TESTING THIS: IF THE LAST FILESEIZE IS ZERO CREATE THE ZIP FILE AND THEN EXIT
